...
// One lookup table for how each category is colored, shared by the
// technology cards and the "Your Stack" list so they always match.
      ...
      {open && (
        <div className="border-t border-line bg-surface px-4 py-4 md:hidden">
          <ul className="flex flex-col gap-1 text-sm font-medium text-ink/80">
            ...
            <li className="pt-2">
              <a href="#signin" className="block rounded-lg px-2 py-2 hover:bg-ink/5">
                Sign In
              </a>
            </li>
          </ul>
...
function TechnologyCard({ tech, isAdded, onAdd }) {
  ...
  // Ekhane amra condition set korechi jodi card ta 'added' thake, tahole pink color ebong shadow ashbe
  const cardStyle = isAdded
    ? 'border-pink-500 bg-pink-50 shadow-lg shadow-pink-200'
    : 'border-line bg-surface shadow-card hover:shadow-md'
...
/* ===================================================================
   Technology Grid — loads data from the JSON file with a loading state
=================================================================== */
