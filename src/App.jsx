        ...
        <div className="border-t border-line bg-surface px-4 py-4 md:hidden">
          <ul className="flex flex-col gap-1 text-sm font-medium text-ink/80">
            ...
            <li className="pt-2 border-t border-line mt-2">
              <a href="#signin" className="block rounded-lg px-3 py-2 hover:bg-ink/5" onClick={() => setOpen(false)}>
                Sign In
              </a>
            </li>
          </ul>
      ...
      <div className="grid items-center gap-10 lg:grid-cols-2">
        <div>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <a href="#technologies" className="btn-primary">Explore Technologies</a>
...
function Loader() {
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-20 text-muted">
      <span className="h-9 w-9 animate-spin rounded-full border-2 border-ink/10" style={{ borderTopColor: '#EC4899' }} />
  ...
  return (
    <article className={`group flex flex-col rounded-2xl border p-5 transition-all duration-300 ${cardStyle}`}>
      <div className="flex items-start justify-between">
        <img src={ICONS[icon]} alt="" className="h-10 w-10 rounded-lg object-contain transition-transform duration-300 group-hover:scale-110" />
        {badge && (
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 text-xs">
        <span className={`rounded-full px-2.5 py-1 font-semibold ${categoryClass(category)}`}>{category}</span>
      ...
      <button
        ...
        onClick={() => onAdd(tech)}
        disabled={isAdded}
        className={`mt-4 w-full rounded-xl py-2.5 text-sm font-semibold transition-colors ${isAdded ? 'bg-pink-500 text-white cursor-default' : 'bg-gray-900 text-white hover:bg-gray-800'}`}
      >
...
function YourStack({ stack, onRemove, onRemoveAll }) {
  ...
  return (
    <aside className="rounded-2xl border border-line bg-surface p-5 shadow-card lg:sticky lg:top-24">
      ...
      {count === 0 ? (
        <p className="mt-6 text-sm text-muted">Your stack is empty. Add technologies to get started.</p>
      ) : (
        <>
          <ul className="mt-4 flex flex-col gap-2">
            {stack.map((tech) => (
              <li key={tech.id} className="flex items-center gap-3 rounded-xl border border-line px-3 py-2.5">
                <img src={ICONS[tech.icon]} alt="" className="h-7 w-7 shrink-0 rounded-md object-contain" />
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-ink">{tech.name}</p>
                  <span className={`mt-0.5 inline-block rounded-full px-2 py-0.5 text-[10px] font-semibold ${categoryClass(tech.category)}`}>{tech.category}</span>
                </div>
                <button type="button" onClick={() => onRemove(tech.id)} className="shrink-0 rounded-full p-1.5 text-ink/40 hover:bg-ink/5 hover:text-ink">
                  <RemoveIcon />
                </button>
              </li>
            ))}
          </ul>
          <button type="button" onClick={onRemoveAll} className="mt-4 w-full rounded-xl border border-rose-200 bg-rose-50 py-2.5 text-sm font-semibold text-rose-600 transition-colors hover:bg-rose-100">
            Remove All
          </button>
        </>
      )}
        ...
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_1fr]">
          <div>
            ...
            <div className="mt-4 flex items-center gap-3">
              {SOCIALS.map((s) => (
                <a key={s.name} href={s.href} className="grid h-8 w-8 place-items-center rounded-full border border-line text-ink/60 transition-colors hover:text-ink">
            ...
            <div key={group.title}>
              ...
              <ul className="mt-3 flex flex-col gap-2 text-sm text-muted">
                {group.links.map((link) => (
                  <li key={link}><a href="#" className="transition-colors hover:text-ink">{link}</a></li>
                ))}
              </ul>
            </div>
          ))}
    ...
    <div id="top" className="min-h-screen bg-surface">
      ...
      <section id="technologies" className="mx-auto max-w-7xl px-4 pb-20 sm:px-6 lg:px-8">
        <div className="mb-6 border-b border-dashed border-line pb-6">
          ...
          <p className="mt-1 text-sm text-muted">Pick one technology per category to build your ideal stack.</p>
        </div>
